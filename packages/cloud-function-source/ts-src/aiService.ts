...
import { getHistory, putHistory } from './gcpHistoryTable';
...
/**
 * A SessionService that persists conversation history to Firestore using the gcpHistoryTable logic.
 */
class FirestoreSessionService extends InMemorySessionService {
  ...
  // Hook into appendEvent to save history back to Firestore after each event
...
export async function getGeminiModel(modelName: string, dataStoreIds?: string[]) {
  const project = await getSecretValue('AIBot', 'GOOGLE_CLOUD_PROJECT');
...
export async function formatResponse(responseString: string, botName: string = "AIBot") {
...
export async function _handlePromptCommand(event: PromptCommandPayload): Promise<void> {
  ...
  try {
    ...
    // Initialize custom session service for Firestore persistence hook
    const sessionService = new FirestoreSessionService(event.channel, parentThreadTs, "supervisor");
    ...
    const formattedResponse = await formatResponse(finalResponse, await getSecretValue('AIBot', 'botName'));

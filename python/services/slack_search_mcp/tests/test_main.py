...
async def test_search_tool_logic():
    ...
    with patch("services.slack_search_mcp.main.get_secret_value", new_callable=AsyncMock) as mock_sec:
        mock_sec.return_value = "token123"
        with patch("services.slack_search_mcp.main.create_client_for_token", new_callable=AsyncMock) as mock_client_factory:
            mock_client = mock_client_factory.return_value
            mock_client.auth_test = AsyncMock(return_value={"team_id": "T123"})
            mock_client.team_info = AsyncMock(return_value={"ok": True, "team": {"domain": "test-team", "id": "T123"}})

            with patch("services.slack_search_mcp.main.is_team_authorized", return_value=True):
                with patch("services.slack_search_mcp.main.generate_embeddings", return_value=[0.1]):
                    with patch("services.slack_search_mcp.main.perform_vector_search", return_value=[{"channel": "C1", "ts": "1.1"}]):
                        mock_client.conversations_replies = AsyncMock(return_value={"ok": True, "messages": [{"text": "found it", "ts": "1.1"}]})

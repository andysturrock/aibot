...
async def test_slack_events_authorized_success():
    ...
    with patch("services.aibot_logic.main.verify_slack_request", return_value=True):
        with patch("services.aibot_logic.main.is_team_authorized", return_value=True):
            with patch(
                "services.aibot_logic.main.publish_to_topic", new_callable=AsyncMock
            ) as mock_pub:
                async with AsyncClient(
                    transport=ASGITransport(app=app), base_url="http://test"
                ) as ac:
    ...
    with patch("services.aibot_logic.main.add_reaction", new_callable=AsyncMock):
        with patch("services.aibot_logic.main.remove_reaction", new_callable=AsyncMock):
            with patch("services.aibot_logic.main.get_history", return_value=[]):
                with patch(
                    "services.aibot_logic.main.create_supervisor_agent",
                    ...
                    with patch("services.aibot_logic.main.Runner") as MockRunner:
                        ...
                        async def mock_run_async(*args, **kwargs):
                            yield Event(
                                ...
                                content=types.Content(
                                    role="assistant", parts=[types.Part(text="hello")]
                                ),
                            )
                        ...
                        with patch(
                            "services.aibot_logic.main.post_message",
                            new_callable=AsyncMock,
                        ) as mock_post:
                            with patch(
                                "services.aibot_logic.main.put_history",
                                new_callable=AsyncMock,
                            ):
                                with patch(
                                    "services.aibot_logic.main.create_bot_client",
                                    new_callable=AsyncMock,
                                ) as mock_bot:
                                    mock_bot.return_value.auth_test = AsyncMock(
                                        return_value={"user_id": "B1"}
                                    )
                                    ...
                                    async with AsyncClient(
                                        ...
                                        response = await ac.post(
                                            "/pubsub/worker", json=envelope
                                        )

                                    assert response.status_code == 200
                                    mock_post.assert_called_with(
                                        "C1", "hello", thread_ts="1.1"
                                    )

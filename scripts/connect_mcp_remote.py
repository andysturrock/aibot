...
async def run_test():
    ...
    if id_token_val:
        ...
        try:
            ...
            db = firestore.Client(project=os.environ.get("GOOGLE_CLOUD_PROJECT", "your-gcp-project-id"))
            ...
            if token_doc:
                ...
                logger.error(f"❌ No token found in Firestore for {user_email}. Have you signed in at https://aibot.example.com/auth/login?")
                return
        except Exception as e:
        ...
        if not id_token_val:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"   1. Sign in at https://aibot.example.com/auth/login")
